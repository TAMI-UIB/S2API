...
def load_saved_credentials():
    """Load credentials if remember_me was enabled."""
...
def save_credentials(username, password):
    json.dump({"username": username, "password": password}, open(CRED_FILE, "w"))
...
def ask_for_credentials():
    """Opens login window and returns (username, password)."""

    login_win = tk.Tk()
    login_win.title("Copernicus Login")
    login_win.geometry("350x200")

    # Load saved credentials
    saved_user, saved_pass, remembered = load_saved_credentials()

    # Username field
    tk.Label(login_win, text="Username:").pack(pady=(10,0))
    ...
    remember_var = tk.BooleanVar(value=remembered)
    remember_check = ttk.Checkbutton(login_win, text="Remember me", variable=remember_var)
    ...
    # Submit button
    submit_button = ttk.Button(login_win, text="Login", command=submit)
...
if not username or not password:
    messagebox.showerror("Error", "Login cancelled. Cannot continue.")
    sys.exit(1)
