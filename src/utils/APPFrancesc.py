...
def BandDownloader(aoi_points, search_period_start, search_period_end,
                   max_cloud_cover, n_prods, progress_var, progress_bar):
    ...
    if len(aoi_points) == 1:
        if aoi_points[0] == '':
            print("No points provided. Try again.")
            sys.exit()
        else:
        ...
        print("Non valid format. Try again.")
        sys.exit()
    ...
    if result.shape[0] == 0:
        print("No products matching criteria found. Try again.")
        sys.exit()

    if int(n_prods) > result.shape[0]:
        n_prods = str(result.shape[0])

    print(f"Found {result.shape[0]} product(s).")
    print(f"Downloading {n_prods} product(s).")
...
def run_downloader():
    ...
    threading.Thread(
        ...
        args=(coords, start_date, end_date, max_cloud, n_prods, progress_var, progress_bar),
        daemon=True
    ).start()
...
frame.grid(row=7, column=0, columnspan=2, sticky="w", padx=5, pady=2)
...
progress_bar.grid(row=10, column=0, pady=10)

# Run button
run_button = ttk.Button(root, text="Run Downloader", command=run_downloader)
run_button.grid(row=11, column=0, pady=10)
